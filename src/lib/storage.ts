...
export function savePresentation(presentation: Omit<SavedPresentation, 'id' | 'createdAt' | 'updatedAt'>): SavedPresentation {
...
export function updatePresentation(id: string, updates: Partial<Omit<SavedPresentation, 'id' | 'createdAt'>>): SavedPresentation | null {
  const savedPresentations = getSavedPresentations();
...
export function addDemoPresentation() {
  ...
  const demoPresentation: Omit<SavedPresentation, 'id' | 'createdAt' | 'updatedAt'> = {
    title: 'サンプルプレゼンテーション',
    company: 'デモ株式会社',
    creator: 'システム',
